'use client';
import { fetchCreateFile, fetchDownloadDocX } from '@/helpers/service';
import fileSchema from '@/helpers/schema/filesSchema';
...
const Quiz2Answers = () => {
  ...
  const uploadFile = async (formData, currQuestion) => {
    ...
    if (formData.file.length) {
      ...
      try {
        const response = await fetchCreateFile(fileData)
  ...
  async function downloadSoalDocx() {
    try {
      const response = await fetchDownloadDocX()
